...
APP_SECRET="请换成一段本机私密随机字符串"
```

如果你的 PostgreSQL 用户、密码或数据库名不同，请修改 `.env`。
`APP_SECRET` 用于加密保存在账户设置中的外部服务 API key。请设置后长期保留；如果更换，旧 API key 将无法解密，需要重新保存。

可选：开启全网辅助资料索引。推荐在应用内 `/settings` 页面保存 API key；下面的 `.env` 配置可作为全局回退。
...
GOOGLE_SEARCH_API_KEY="你的 Google Custom Search JSON API Key"
GOOGLE_SEARCH_ENGINE_ID="你的 Programmable Search Engine ID"
```
...
1. 双击 `启动研经笔记.command`
2. 等待脚本检查 Node.js、npm、依赖、`.env`、数据库、迁移和端口
3. 浏览器会自动打开应用
4. 使用期间保持终端窗口打开
5. 关闭时在终端按 `Ctrl+C`

如果应用已经在运行，再次双击启动器会直接打开现有服务，不会再启动第二个 Next.js 进程。

启动器会在失败时保留窗口，并显示卡在哪一步：
...
- 新建文本资料
- 上传并解析 `.txt`、`.md`、`.markdown`、`.pdf`、`.docx`
- 编辑资料标题、类型、标签和正文
- 删除资料
- 搜索资料内容
...
- 使用 Brave Search、Tavily 或 Google Programmable Search 搜索全网资料。
- 根据当前研读经文预填搜索关键词。
- 查看网页标题、来源、摘要和链接。
- 将搜索结果保存为资料库条目。
- 保存后自动关联到当前研读项目。
- API key 可在 `/settings` 保存到当前账户，保存时会加密，页面只显示预览。
...
1. 升级资料检索：全文索引、命中摘要、资料类型和标签筛选优化。
2. 资料关联研读：资料与经文/研读项目关联，研读页显示相关资料。
